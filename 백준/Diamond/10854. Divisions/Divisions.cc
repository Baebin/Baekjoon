...
class IntegralDivisorCalculator {
public:
    explicit IntegralDivisorCalculator(const ull& N) : N(N) {}
    ull get() {
        vector<ull> primes = PollardRho(N).getPrimes();
        sort(primes.begin(), primes.end());
        primes.emplace_back(0);

        ull divisor = 1, cnt = 1, len = primes.size() - 1;
        for (int i = 0; i < len; i++)
            if (primes[i] == primes[i + 1]) cnt++;
            else divisor *= cnt + 1, cnt = 1;
        return divisor;
    }
private:
    ull N;
...
int main() {
    ull N;
    cin >> N;
    cout << IntegralDivisorCalculator(N).get();
}